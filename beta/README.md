...
> **This is the beta build.** It lives at `rbmglobal.co.uk/lz-tool/beta/`. The
> live tool is one level up at `rbmglobal.co.uk/lz-tool/`, and nothing in this
> folder changes it. The header carries a BETA chip; there is no banner.
>
> What is different here: RBM Global branding (see the 6 Sep entry under
> *Changes*). Everything else — the parser, the imagery pipeline, the screen-photo
> reader, the exports — is the same code as the live build.
>
> Two deliberate choices make the two coexist safely on one origin:
> * The OCR pack is loaded from `../vendor/`, so the beta shares the live build's
>   7 MB download instead of duplicating it in the repo.
> * The service worker caches under `lz-brief-BETA-*` and only ever evicts caches
>   with that prefix. Cache storage is per-origin, not per-scope: a blanket
>   "delete what is not mine" would wipe the live tool's offline copy, which is
>   the one thing the service worker exists to prevent.
>
> To promote this to live: copy these files over the repo root, restore
> `vendor/` to `vendor/` in `ocrEngine`, drop the beta banner and chip, and set
> the manifest name back.
...
**6 Sep 2026 (evening, 2)** — RBM Global branding.
Built from `Full RBM CI.pdf` in Rob's logo folder, not from eye.

- **Palette** straight off *Colour Values 01*: `#22222f` ground, `#2c2c3a` panels
  (RBM Dark, PANTONE Black C), `#f0f0f0` ink, `#7e7e87` tertiary, with the only
  two chromatic accents in the brand — teal `#6b8f8f` and sand `#e6e0d4`. The old
  `--accent` was `#ffe400`; there is no yellow anywhere in the RBM palette.
- **Where the brand grey is too dark to use.** `#7e7e87` on `#2c2c3a` is 3.4:1,
  below the 4.5 a crew needs in daylight, so secondary text uses a lightened
  `#a2a2ad` and the brand grey is kept for decorative marks. Accent *text* uses a
  teal tint `#93b9b9` (6.5:1); the full brand teal is used only as a fill, where
  contrast is not a reading question. Every pair was measured, not eyeballed —
  lowest text ratio on the page is 5.0:1.
- **The emblem is traced, not redrawn.** `Icon.pdf` was measured: three arcs from
  one centre, all starting due south, sweeping **270° / 180° / 90°** at radii
  18.4 / 32.9 / 47.1, stroke 7.33 — the stroke width and the gap between rings
  are the same number. It ships as 343 bytes of inline SVG using `currentColor`
  and the CI's own 100 / 67 / 30 % ring opacities, so it needs no recolouring on
  any ground and adds no request. Home-screen icons are regenerated from it, on
  brand dark, matching *Profile Image Options, option 2*.
- **No gradients, no drop shadows** anywhere — the CI forbids both on the
  identity, and the same restraint reads better on the rest of the page. The
  sticky header's gradient is now a flat ground and a hairline.
- **Type**: Montserrat, then Avenir Next, then the platform default. Both named
  faces are brand fonts (Gotham, the title face, is licensed and not a webfont).
  **No webfont is loaded** — this page still fetches nothing, and a crew with no
  signal must still get it. On an iPhone or iPad that resolves to Avenir Next,
  which *is* the brand's subtitle face. Brand tracking 20px ≈ `.02em`.
- **The imagery is branded only where it is safe to brand it.** Two palettes now
  exist side by side and the code says so. Bars, labels, cards and the detail
  card follow RBM. Everything drawn **on the ground** keeps the colour that
  survives over terrain: the red reticle, the magenta rings, the yellow
  next-view box, the black casings, the grid. Grey and teal disappear over scrub;
  that is the whole reason the originals are the colours they are.
  Proved, not asserted: rendering the same step before and after gives
  **356,338 changed pixels, 0 of them inside the map area.**
- The blue Sentinel chip and the amber ENLARGED chip are also drawn over the
  ground, so both were left alone. Say the word if you want the Sentinel one
  moved to teal.
- No logo on the exported images, at Rob's request — branding lives in the app.
- `sw.js` → `lz-brief-v10`; manifest and `theme-color` → `#22222f`.
